...
| Router Deep preset | `${DSH_HOME:-~/.dsh}/.agent-presets/router-deep/` | versions.json（由 router-spec 改名） |
...
1. 新会话选择 `Router Standard (experimental)` 或 `Router Deep (experimental)` preset
...
> `dev_router_status` 是 Router preset 的 agent-plane 工具，须在选择了对应 router preset 的新会话中使用。
